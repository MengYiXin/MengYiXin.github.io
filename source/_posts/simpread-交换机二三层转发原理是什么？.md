...
![](https://pic4.zhimg.com/v2-2e9606f05c1c778ac23df0ae3ac699ec_r.jpg?source=1940ef5c)交换机是如何添加、更新、删除 MAC 地址表条目的？  
...
![](https://pic1.zhimg.com/v2-94c4fdb3a308f405a7b6cc7d7b02ca33_r.jpg?source=1940ef5c)
...
![](https://pic1.zhimg.com/v2-182787b10366b2981075f003c4b1f6f1_r.jpg?source=1940ef5c)
...
![](https://pic2.zhimg.com/v2-d2152dead545b7b6b2232fb7293a0f72_r.jpg?source=1940ef5c)

1.  交换机的 MAC 地址表中有这个数据帧的目的 MAC 地址，且对应端口不是接收到这个数据帧的端口，交换机知道目的设备连接在哪个端口上，因此交换机会根据 MAC 地址表中的条目将数据帧从对应端口单播**转发**出去，而其它与交换机相连的设备则不会收到这个数据帧。

![](https://pic3.zhimg.com/v2-5bdf925ac01bdef2d090c1e914c3653c_r.jpg?source=1940ef5c)

1.  交换机的 MAC 地址表中有这个数据帧的目的 MAC 地址，且对应端口就是接收到这个数据帧的端口。这种情况下，交换机会认为数据帧的目的地址就在这个端口所连接的范围内，因此目的设备应该已经收到数据帧。这个数据帧与其它端口的设备无关，不会将数据帧从其它端口转发出去。于是，交换机会**丢弃**数据帧。

![](https://pica.zhimg.com/v2-90264b0501dff33c72cc6ce16eec8470_r.jpg?source=1940ef5c)
...
![](https://pic2.zhimg.com/v2-a24da6313ad461a0d5be1e451c075b84_r.jpg?source=1940ef5c)![](https://pic1.zhimg.com/v2-58cc62235600bb40115f969a8776795d_r.jpg?source=1940ef5c)
...
查找匹配的路由条目，就需要将数据包的目的 IP 地址与各个路由条目的网段地址先进行二进制**与（ AND ）运算**，再将运算结果与路由条目的网段地址进行**比较**，若一致则该条目与目的 IP 地址相匹配。最后，与所有路由条目完成运算和比较，可得到一条或多条相匹配的路由条目。也可能没有匹配的路由条目，那么丢弃数据包。

![](https://pic1.zhimg.com/v2-6ec2a524c061ae8b2790ef6d5f4ecea9_r.jpg?source=1940ef5c)

1.  从多个匹配项中选择**掩码最长**的路由条目。

如果路由表中有多条路由条目都匹配数据包的目的 IP 地址，则路由器会选择掩码长度最长的路由条目，这种匹配方式称为**最长匹配原则**。

例如：10.1.3.10 的网络地址与 10.1.3.0/16 和 10.1.3.0/24 两项都匹配，这时应该选择匹配度最长的 10.1.3.0/24 。

![](https://pic1.zhimg.com/v2-6cb2b1e3d2670718fe04fe0a32308d34_r.jpg?source=1940ef5c)

1.  将数据包按照相应路由条目进行**转发**。

路由条目中包含**下一跳**和**出接口**。当路由器找到相应的路由条目后，它就会根据对应的下一跳和出接口，将数据包从出接口发送数据给下一跳设备。

![](https://pic1.zhimg.com/v2-04d3188c4c2e1323eff102b5ad7a51c6_r.jpg?source=1940ef5c)
...
![](https://pic2.zhimg.com/v2-37aeac84e9712213b7190e3eeba4d182_r.jpg?source=1940ef5c)
...
![](https://pic3.zhimg.com/v2-c70adbd7b5475c1592a5a7be1b39029b_r.jpg?source=1940ef5c)
...
![](https://pic3.zhimg.com/v2-cf3fe01d0226b3e51142683f870cc8ea_r.jpg?source=1940ef5c)

1.  **更新 ARP 缓存表**：主机 A 收到 ARP 响应报文后，将主机 C 的 IP 地址和 MAC 地址记录到 ARP 缓存表中。下次再向主机 C 发送数据时，直接将缓存的目的 MAC 地址进行封装。

![](https://pic1.zhimg.com/v2-3321c6c00c0240c0ae17d51285b0f628_r.jpg?source=1940ef5c)

当主机 A 向**不同网段**的主机 C 发送数据，但是不知道主机 C 的 MAC 地址。

![](https://pic1.zhimg.com/v2-b3d81353dc43a4c8e3b909593ce7c2ce_r.jpg?source=1940ef5c)

1.  主机 A 使用主机 C 的 IP 地址查询 ARP ，ARP 发现主机 C 不在同一个网段，需要通过**默认网关**（即默认路由的下一跳地址），但是没有网关 MAC 地址；

![](https://pica.zhimg.com/v2-79ac25f0a620da7a241dbf8c1149b281_r.jpg?source=1940ef5c)

1.  主机 A 先将发送给主机 C 的数据放入缓存中，然后发送 ARP 请求报文，主机 A 以网关 IP 地址为目的 IP 地址发送 **ARP 广播请求报文**；

![](https://pic3.zhimg.com/v2-0669e4d3be8d9567b86a0a2e3422733e_r.jpg?source=1940ef5c)

1.  路由器收到 ARP 广播请求报文后，将主机 A 的 MAC 地址和对应端口添加到自己的 MAC 表中，然后查看目的 IP 地址发现是请求自己的 MAC 地址，于是单播发送 **ARP 响应报文**；

![](https://pic2.zhimg.com/v2-55efd7c1c25f219fa6696bc703a71b53_r.jpg?source=1940ef5c)

1.  主机 A 收到 ARP 响应报文后，将发送给主机 C 的数据封装**网关 MAC 地址**为目的 MAC 地址进行发送；

![](https://pic2.zhimg.com/v2-48f6c4e4e29e7fad6dc3fb21363abca0_r.jpg?source=1940ef5c)
...
![](https://pica.zhimg.com/v2-ad1e82bed0952190f5d3d0613faf1566_r.jpg?source=1940ef5c)

1.  主机 C 收到 ARP 请求报文后，添加路由器的端口和 MAC 地址到 MAC 地址表，单播发送 **ARP 响应报文**；

![](https://pic1.zhimg.com/v2-b435e0be0d9287ce9271d574d352ff0a_r.jpg?source=1940ef5c)

1.  路由器收到主机 C 的 MAC 地址后，将其添加到 MAC 地址表中。将主机 A 发送给主机 C 的报文重新封装，以自己的 MAC 地址为源 MAC 地址，以主机 C 的 MAC 地址为目的 MAC 地址，发送给主机 C ；

![](https://pic1.zhimg.com/v2-374c5ac6cd0b4489b30cc94e9e5ebbcc_r.jpg?source=1940ef5c)

1.  主机 C 收到主机 A 发送的数据，发送过程结束。

当主机 C 向主机 A 发送回复报文时，同主机 A 向主机 C 发送数据的步骤一致。

![](https://pic3.zhimg.com/28b870d08_xs.jpg?source=1940ef5c)小耸​
...
![](https://pica.zhimg.com/v2-1b5968078243a87c425788cc06159a61_xs.jpg?source=1940ef5c)sir long
...
![](https://pic2.zhimg.com/v2-4a4c9ee509c1b56a6de3850318d17493_r.jpg?source=1940ef5c)
...
![](https://pic2.zhimg.com/v2-5e13ff74b407544c96ca8e909ee854f2_r.jpg?source=1940ef5c)

关于 ARP 报文各个字段的含义，用 wireshark 抓一个 ARP 包来查看，如下所示：

![](https://pic1.zhimg.com/v2-bb499942583323d698ea39518561b9a5_r.jpg?source=1940ef5c)
